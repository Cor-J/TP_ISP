...
def get_generator(resume=True):
    '''Get the prod variable at the last moment'''
    if resume:
        try:
...
def get_best_result(file='h1_I.txt'):
    try:
        with open(file,'r',encoding='utf8') as f:
            raw_text = [line.replace('\n','') for line in f.readlines()]
            return {'sentence':raw_text[0], 'hash':raw_text[1], 'nb_matching_bytes':int(raw_text[2])}
    except FileNotFoundError:
        return {'sentence':'', 'hash':'', 'nb_matching_bytes':0}

def save_result(result, file='h1_I.txt', mode='w'):
    with open(file, mode) as f:
        f.write(result['sentence'] + '\n' + result['hash'] + '\n' + str(result['nb_matching_bytes']))

def compare_hexhash(h1, h2):
    """Return number of corresponding bytes"""
    same_bytes = 0
    for ptr in range(0, len(h1),2):
        if h1[ptr:ptr+2] == h2[ptr:ptr+2]:
            same_bytes += 1
        else:
            return same_bytes
    return same_bytes
...
if __name__ == '__main__':
    ...
    original_hash = md5('F*ck you, dear professor!'.encode('utf8')).hexdigest()
    result = {'sentence': '', 'hash': '', 'nb_matching_bytes': 0}
    ...
    try:
        for bad, beautiful, big, boring, colorname, few, good, interesting, look, old, tell in prod:
            result['sentence'] = f"{good.capitalize()} news or {bad} news? I'm gonna {tell} you and let you judge! "\
            ...
            result['hash'] = md5(result['sentence'].encode('utf8')).hexdigest()
            result['nb_matching_bytes'] = compare_hexhash(result['hash'], original_hash)

            if result['nb_matching_bytes'] > best_result['nb_matching_bytes']: # while no results are better than the previous one, loop continue
                save_result(result, f"log_{timestamp}.txt", 'a')
                save_result(result)
                ...
                while True:
                    print('\n\nSentence:\n', result['sentence'],
                          '\nSentence hash: ', result['hash'],
                          '\nOriginal hash: ', original_hash,
                          '\nNb of matching bytes: ',  result['nb_matching_bytes'])
                    rep = input('\nDo you  want to continue to try to find another hash? y or n\n'
                                '"y" will write log this result into result_log.txt and try to find a best one "'
                                '"n" will the best result into h1_I.txt\n').lower()
                    if rep == 'y':
                        best_result = deepcopy(result) # avoid modification of result impact best_result and reciprocally
